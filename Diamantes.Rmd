...
Se observa que ninguna de las variables se ajusta bien a la densidad de una distribución normal, aunque la variable _depth_ tiene una forma similar aunque más punteaguda y con colas más pesadas (por las estadísticas descriptivas) lo que podría aproximarse a través de una _t-student_.
...
De las tablas anteriores se muestra nuevamente que el Color es la variable que más se relaciona con el precio, mientras que el corte y la claridad no parecen tener una relación tan directa. En particular, el Corte _Fair_ tiene un precio promedio mayor al Corte _Good_, veamos is esto es estadísticamente significativo: 
